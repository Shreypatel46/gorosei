...
while True:
    ...
    if hands:
        for hand in hands:
            ...
            for id,landmark in enumerate(landmarks):
                x=int(landmark.x *frame_width)           #  will return decimal points we need pixel points so multiply by frame width
                y=int(landmark.y *frame_height)          #  will return decimal points we need pixel points so multiply by frame height
    ...
    if cv.waitKey(15) & 0xFF==ord('q'):     #says if the frame is there for 15 sec or if d is pressed break out of the video
        break
